import { Pagination, PaginationContent, PaginationItem, PaginationEllipsis } from "@frontend/components/ui/pagination"
import { ColumnDef, PaginationState, PaginationOptions, SortingState, OnChangeFn, useReactTable, getCoreRowModel} from "@tanstack/react-table"
import { Button } from "@frontend/components/ui/button"

import { CourseProposal, CourseProposalVisibility } from "@frontend/modules/enrollment/interfaces/CourseProposal"
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger } from "@frontend/components/ui/accordion"
  ...
  export default function DataAccordion<EnrollmentProposal extends Record<string, any>>({
    data,
    columns,
    pagination,
    paginationOptions,
                    ...
                    <AccordionItem key={row.id} value={row.id}>
                      <AccordionTrigger>
                        <div className="w-full px-2 flex justify-between">
                          <h3>{row.original.courseName}</h3>
                          <p>
                            {row.original.schedules.length} {row.original.schedules.length.length === 1 ? 'horario' : 'horarios'}
                          </p>
                        </div>
                      </AccordionTrigger>
                      <AccordionContent>
                        <ul className="flex flex-col gap-2">
                            {row.original.schedules.map((courseproposal: CourseProposal, idx: number) => (
                            <li className="flex justify-between " key={courseproposal.courseId}>
                                <div className="grid grid-cols-12 items-center border-b py-2 flex-grow">
      <span className="font-semibold col-span-9">Horario N°{idx}</span>
      <span className=" col-span-1">{courseproposal.vacants} vacantes</span>
      <span className=" col-span-1">{courseproposal.visibility===CourseProposalVisibility.visible?
                                                            'Visible':'Oculto'}</span>
      <span className=" col-span-1 flex flex-row-reverse"><Button className="mx-5" variant={'outline'}>
                                    Eliminar
                                </Button></span>
