...
//import UploadStudentsInCourseDialog from './components/UploadStudentsInCourseDialog'
...
export default function ScheduleManagement() {
  ...
  const handleGetAccounts = async () => {
    ...
    const selectedSchedule = schedules?.find(
      (schedule) => schedule.id.toString() === scheduleCode
    )
    if (selectedSchedule) {
  ...
  return (
    <div className="flex flex-col my-6 p-6 rounded-lg shadow-md">
      <div className="flex flex-col  gap-6 items-start md:items-left ">
        <div className="flex flex-col md:flex-row gap-6 w-full md:w-3/4 space-y-1.5 md:items-end">
          ...
          <UploadCSVDialog courseCode={course?.code} />
        </div>
        ...
        <div className="w-full md:w-1/2 space-y-1.5">
          ...
          <div className="flex gap-2">
            <Select
              onValueChange={setScheduleCode}
              disabled={!course || isSchedulesLoading}
            >
              <SelectTrigger className="w-full">
                <SelectValue
                  placeholder={
                    isSchedulesLoading
                      ? 'Cargando horarios...'
                      : 'Seleccione un curso para obtener horarios'
                  }
                />
              </SelectTrigger>
              <SelectContent>
