...
export const AccountTableColumns = (
  courseCode: string,
  scheduleCode: string
): ColumnDef<AccountRoles>[] => {
  const baseColumns: ColumnDef<AccountRoles>[] = [
    {
      accessorKey: 'code',
      header: ({ column }) => (
        <Button
    ...
    {
      accessorFn: (row) =>
        `${row.name} ${row.firstSurname} ${row.secondSurname}`,
      header: 'Nombre',
    },
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const { mutate: toggleLead } = useMutation({
          mutationFn: () =>
            ScheduleService.toggleLead(id, courseCode, scheduleCode), //TODO: ACA DEBERIA PASAR EL courseCode,scheduleCode del componente padre
          onSuccess: () => {
            queryClient.invalidateQueries({
              queryKey: [QueryKeys.users.GENERIC],
            })
